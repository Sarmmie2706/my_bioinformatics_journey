...
# Creates a list of dictionaries where each dictionary holds each sequence's is, length, 
# gc_fraction, motif counts, and molecular weight 
record_list = []
...
# %% 
# Print a table-like object using the key-value pair of each dictionary in the list
...
# %%
# Calculating and printing the summary statistics
...
# Calculating the longest and shortest sequences
max_length = 0
max_id = ""
for record in record_list:
...
min_length = 160000000000 #Size of the largest genome
min_id = ""
for record in record_list:
    if record["length"] < min_length:
        min_length = record["length"]
        min_id = record["id"]
print(f"Shortest: {min_length} bp ({min_id})")

total_gc = 0
for record in record_list:
    total_gc += record["gc_fraction"]
ave_gc = round((total_gc/len(record_list)),2)
print(f"Average GC: {ave_gc}%")


# %%
# Write the important statistics to a csv file
...
# %%
# Writing the important statistics to a text file
